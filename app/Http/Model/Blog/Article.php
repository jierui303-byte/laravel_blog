...
//        echo '</pre>';
    ...
    //获取某个顶级栏目下及其子栏目下所有文章  参数只能是顶级栏目ID
    public function getTopCateArticle($cate_pid, $currentTime, $num){
        //获取顶级栏目下所有子栏目的文章
        $topNavArticles['data'] = $this->orderBy('art_dingTime', 'desc')->where('art_status', '1')->where('cate_pid', $cate_pid)->where(function($q1) use($currentTime){
            $q1->orWhere('art_dingTime', '<', $currentTime);
        })->take($num)->get(['art_id', 'art_title', 'art_description', 'art_time', 'art_dingTime', 'art_content']);
        //获取该栏目的栏目名称
        $topNavArticles['cate_name'] = (new Category())->getCategoryName($cate_pid);
        $topNavArticles['cate_id'] = $cate_pid;
//            var_dump($topNavArticles);
        return $topNavArticles;
    }
...
//            })->take(1)->get();//paginate分页
    ...
    public function searchKeywords($keyword, $pageNum)
    {
        ...
        $data = Article::leftJoin('category', 'article.cate_id', '=', 'category.cate_id')
            ->where(function($q1) use($currentTime){
                $q1->orWhere('article.art_dingTime', '<', $currentTime);
            })->where('article.cate_pid', 13)
            ->where('article.art_title', 'like','%'.$keyword.'%')
            ->where('article.art_content', 'like','%'.$keyword.'%')
            ->where('article.art_status', '1')
            ->orderBy('article.art_dingTime', 'desc')
            ->paginate($perPage = 2, $columns = ['*'], $pageName = 'page', $page = $pageNum);//paginate分页
    ...
    //获取文章的上一篇下一篇
    public function getArticlePrevNext()
    {
        $currentTime = time();
    ...
    //获取所有文章用来进行收录状况查询
    public function getAllArticleCheckSouLu()
    {
        //每次获取1000条数据 查询并更新收录状态
        $data = $this->where('art_status', '1')->take(10)->get();
