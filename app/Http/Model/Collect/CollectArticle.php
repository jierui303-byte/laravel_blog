...
{
    protected $table = 'collect_article';
    ...
    public function getAllTopCategoryArticle()
    {
        //声明一个变量
        $topData = array();

        //获取所有的顶级分类ID及其分类名称
        $topNav = (new Category())->getAllTopCategoryId();//所有开启状态的顶级分类id
        foreach($topNav as $k => $value){
            //获取顶级栏目下所有子栏目的文章
            $topNavArticles = $this
                ->orderBy('art_id', 'desc')
                ->where('cate_pid', $value['cate_id'])
                ->where('art_status', '1')
                ->take(15)
                ->get([
                    'art_id',
...
//            var_dump(count($topNavArticles));
            $count = count($topNavArticles->map(function ($value) {
                return (array)$value;
            })->toArray());
            if($count > 0){
                $topData[$k]['cate_id'] = $value['cate_id'];//顶级栏目id
                $topData[$k]['cate_math'] = $value['cate_math'];//顶级栏目域名缩写
                $topData[$k]['cate_domain'] = $value['cate_domain'];//顶级栏目域名
                $topData[$k]['cate_name'] = $value['cate_name'];//顶级栏目名称
                $topData[$k]['articleList'] = $topNavArticles;//顶级栏目下的文章列表
            }
        }
