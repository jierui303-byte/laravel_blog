...
class PicturesController extends CommonController
{
    ...
    public function __construct(Request $request)
    {
        ...
        $this->picturesData = new Pictures();
        $this->currentTime = time();
    }

    public function index()
    {
        $data = $this->picturesData->orderBy('pic_id', 'desc')->get();
        $count = count($data);
        return view('admin.pictures.show', compact('data', 'count'));
    }
    ...
    public function create()
    {
        if($this->request->getMethod() == 'POST'){
            ...
            if ($validator->passes()) {
                //通过article模型入库
                $re = $this->picturesData->create($data);

                if ($re) {
                    return $this->success('添加成功!', 'admin/pictures');
                } else {
                    return $this->error('数据填充失败，请稍后再试!', 'admin/pictures');
                }
            } else {
                return $this->error($validator->customMessages, 'admin/pictures');
            }

        }else{
            $data = $this->subWebTypeData->tree();//获取分类列表
            return view('admin.pictures.add', compact('data'));
        }
    }
    ...
    public function edit($id)
    {
        if($this->request->getMethod() == 'PUT'){
            ...
            $re = $this->picturesData->where('link_id', $id)->update($data);
            if ($re) {
                return $this->success('修改成功!', 'admin/pictures');
            } else {
                return $this->error('数据更新失败，请稍后再试!', 'admin/pictures');
            }
        }else{
            $data = $this->subWebTypeData->tree();
            $field = $this->picturesData->find($id);

            return view('admin.pictures.edit', compact('data', 'field'));
        }
    ...
    {
        $res = $this->picturesData->destroy($id);
        if ($res) {
            $this->success('删除成功', 'admin/pictures');
        } else {
            $this->error('删除失败', 'admin/pictures');
        }
    }
    ...
    public function del_all()
    {
        ...
        $ids = explode(',', $ids);
        $res = $this->picturesData->destroy($ids);
        if ($res) {
            $this->success('删除成功', 'admin/pictures');
        } else {
            $this->error('删除失败', 'admin/pictures');
