...
<article class="page-container">
    <form action="" method="post" class="form form-horizontal" id="my-form-submit">
            ...
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
					<select name="link_web_type_id" id="link_web_type_id" class="select">
                        ...
                        @foreach($data as $k=>$d)
                            ...
                            @else
                                @if($d->type_id == $field->link_web_type_id)
                                    <option value="{{$d->type_id}}" selected>{{$d->_type_name}}</option>
                                @else
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9 skin-minimal" id="resText" data-ids="{{$field->link_web_id}}">
                {{--<div class="check-box">--}}
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{$field->link_name}}" placeholder="" id="link_name" name="link_name">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">链接标题：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{$field->link_title}}" placeholder="" id="link_title" name="link_title">
            </div>
        </div>
        <div class="row cl">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" name="link_start_time" value="{{$field->link_start_time}}" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}'})" id="datemin" class="input-text Wdate">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">有效结束日期：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" name="link_end_time" value="{{$field->link_end_time}}" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'datemin\')}'})" id="datemax" class="input-text Wdate">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">排序：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{$field->link_order}}" placeholder="" id="link_order" name="link_order">
            </div>
        </div>
        <div class="row cl">
...
<script type="text/javascript">
    ...
    function initType() {
        $.ajax({
            type: "get",
            url: "{{url('admin/links/getWebListByWebTypeId')}}"+'/{{$field->link_web_type_id}}',
            dataType: "json",
            success: function(data){
                ...
                $('#resText input').iCheck({
                    ...
                    var link_web_id_array = $('#resText').data('ids');
                    link_web_id_array = link_web_id_array.split(",");
                    if(in_array($(this).val(), link_web_id_array)){
                        $(this).iCheck('check');
                    }else{
                        $(this).iCheck('uncheck');
                    }
    ...
    $(function(){
        ...
        $("#my-form-submit").validate({
            ...
            submitHandler:function(form){
                $(form).ajaxSubmit({
                    url:"{{url('admin/links/edit', ['id'=>$field->link_id])}}",//form提交数据的地址  data数据不用带了自动加上了
